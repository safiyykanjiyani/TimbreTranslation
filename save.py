...
def stftTransfer(reciever, donor, sine, framerate, size1, N1, size2, N2, hop, smoothing, transfer):
	...
	sineAnalysisWindow = (int(math.floor((size2.size+1)/2)), int(math.floor(size2.size/2)))
	sineHop = int(sine.size/length) # hop size for donor
	sine = np.append(np.zeros(sineAnalysisWindow[1]), sine)
	sine = np.append(donor, np.zeros(sineAnalysisWindow[0]))
	sineSave = sineAnalysisWindow[0]
	...
	for i in range(length):
		...
		sineProgress1 = sineSave - sineAnalysisWindow[0]
		sineProgress2 = sineSave + sineAnalysisWindow[1]
		...
		synthesis = transfer * donorMagnitude + (1 - transfer) * recieverMagnitude #- (1 - transfer) * sineMagnitude
